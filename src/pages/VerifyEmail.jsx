...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    if (token && !hasCalled.current) {
      ...
      setStatus("error");
      setMessage("NO VERIFICATION KEY DETECTED.");
    }
  }, [token, verifyEmail]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-bg-primary relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-bg-secondary border border-border-subtle rounded-[2.5rem] p-10 text-center animate-fade-in shadow-2xl relative z-10">
        <div className="mb-10">
           <h1 className="text-2xl font-display font-black text-text-primary tracking-tight">Security Check</h1>
           <p className="text-[10px] font-bold text-text-muted uppercase tracking-[0.2em] mt-2">Identity Validation</p>
        </div>
        ...
        <div className="min-h-[200px] flex flex-col justify-center">
          {status === "verifying" ? (
            <div className="flex flex-col items-center py-6">
              ...
              <p className="text-[10px] font-black text-text-muted uppercase tracking-widest animate-pulse">Scanning Credential...</p>
            </div>
          ) : status === "success" ? (
            <div className="flex flex-col items-center animate-fade-in py-6">
              ...
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest leading-loose">Access to the workspace is now granted.</p>
              <Link to="/login" className="btn-primary w-full py-4 mt-8 font-bold uppercase tracking-widest text-[11px] shadow-blue">
                Proceed to Login
              </Link>
            </div>
          ) : (
            <div className="flex flex-col items-center animate-fade-in py-6">
              ...
              <Link to="/register" className="text-accent-primary hover:text-accent-primary/80 font-black text-[10px] uppercase tracking-widest transition-all">
                Restart Enrollment
              </Link>
            </div>
          )}
